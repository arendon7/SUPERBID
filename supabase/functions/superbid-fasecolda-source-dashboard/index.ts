...
function login(error=false,lot=""){
  return html("SUPERBID — Source Research",`<form class="login" method="post" action="/functions/v1/superbid-fasecolda-source-dashboard/login">${lot?`<input type="hidden" name="lot" value="${esc(lot)}">`:""}<div class="ey">SUPERBID · v0.57.1</div><h1>Fasecolda Source Research</h1><p class="sub">Acceso privado. La prioridad usa únicamente metadata de fuentes registradas; no inspecciona el contenido ni homologa vehículos.</p>${lot?`<p class="sub">Continuar con lote ${esc(lot)} después de autenticar.</p>`:""}${error?'<p class="err">Credencial inválida.</p>':""}<input type="password" name="password" autocomplete="current-password" required placeholder="Clave de acceso"><button class="primary">Entrar</button></form>`,error?401:200);
}
