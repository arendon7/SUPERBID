...
function login(error=false,lot=""){
  return html("SUPERBID — Source Research",`<form class="login" method="post" action="/functions/v1/superbid-fasecolda-source-dashboard/login">${lot?`<input type="hidden" name="lot" value="${esc(lot)}">`:""}<div class="ey">SUPERBID · v0.57</div><h1>Fasecolda Source Research</h1><p class="sub">Acceso privado. La prioridad usa únicamente metadata de fuentes registradas; no inspecciona el contenido ni homologa vehículos.</p>${lot?`<p class="sub">Continuar con lote ${esc(lot)} después de autenticar.</p>`:""}${error?'<p class="err">Credencial inválida.</p>':""}<input type="password" name="password" autocomplete="current-password" required placeholder="Clave de acceso"><button class="primary">Entrar</button></form>`,error?401:200);
}
...
async function board(req:Request){
  ...
  let q=`/rest/v1/dashboard_fasecolda_source_research_priority_v57?select=${encodeURIComponent(sel)}&source_research_actionable=eq.true&order=research_rank.asc,closes_at.asc&limit=500`;
  ...
  const all=await rows(`/rest/v1/dashboard_fasecolda_source_research_priority_v57?select=${encodeURIComponent("external_lot_id,research_route,disposition_status")}&source_research_actionable=eq.true&limit=500`);
  const cards=[
    ["Casos accionables",all.length],
    ["Identidad primaria",all.filter((x:any)=>x.research_route==="REVIEW_IDENTITY_PRIMARY_SOURCE").length],
    ["Identidad secundaria",all.filter((x:any)=>x.research_route==="REVIEW_IDENTITY_SECONDARY_SOURCE").length],
    ["Peritaje potencial",all.filter((x:any)=>x.research_route==="REVIEW_PERITAJE_FOR_IDENTITY_FACTS").length],
    ["Adquirir fuente",all.filter((x:any)=>x.research_route==="ACQUIRE_EXTERNAL_IDENTITY_SOURCE").length],
...
async function detail(req:Request,lot:string){
  ...
  const inventory=objArr(x.source_inventory);
  ...
  const firstReview=safeHttpUrl(x.first_review_source_url);
  const selected=requested&&seen.has(requested)?requested:firstReview&&seen.has(firstReview)?firstReview:"";
