...
import {
  ...
  type CandidateDiscriminatorEntry,
  type StructuredDiscriminatorKey,
...
const DISCRIMINATOR_LABELS: Record<StructuredDiscriminatorKey, string> = {
  engine_cc: "MOTOR",
  transmission: "TRANSMISIÓN",
  drivetrain: "TRACCIÓN",
  fuel: "COMBUSTIBLE",
...
main{padding:20px 26px 45px;max-width:1900px;margin:auto}.nav,.actions,.filters,.hint-summary,.disc-summary,.token-list{display:flex;gap:7px;flex-wrap:wrap;align-items:center}
...
.pill{font-size:10px;font-weight:800;border-radius:99px;padding:5px 8px;display:inline-block}.reviewed,.match,.hint-consistent,.disc-active{background:#daf0e1;color:#175935}.draft,.ambiguous,.notstated,.hint-unknown{background:#fff0c9;color:#72510a}.none{background:#ecefec;color:#586158}.conflict,.blocked,.hint-differs{background:#f4dfdf;color:#7f2424}.medium,.hint-compatible,.token{background:#e8eef3;color:#425c70}
