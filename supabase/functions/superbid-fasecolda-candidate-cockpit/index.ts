...
import {
  ...
  compareVehicleIdentityHints,
  extractVehicleIdentityHints,
  type HintComparison,
} from "./identity_hints.ts";
...
const DIMS = [
  ...
  ["engine_cc", "Motor / cilindraje", "Use solo lo declarado por la fuente; no infiera por el código candidato."],
  ["transmission", "Transmisión", "MT/AT/TP u otra denominación sustentada por la fuente."],
  ["fuel", "Combustible", "Gasolina, diésel, híbrido, eléctrico u otro solo si está declarado."],
  ["drivetrain", "Tracción", "4x2, 4x4, AWD u otra solo si está sustentada."],
  ["trim_body_use", "Versión / carrocería / uso", "Trim, cargo/pasajeros, cabina, puertas u otro discriminador de versión."],
] as const;

const STATUSES = ["", "MATCH", "NOT_STATED", "CONFLICT"];
const HINT_LABELS: Record<string, string> = {
  engine_cc: "Cilindraje",
  transmission: "Transmisión",
  drivetrain: "Tracción",
  fuel: "Combustible",
};

const esc = (v: any) => String(v ?? "").replace(/[&<>"']/g, (m) => ({
  "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;",
}[m] || m));

const cop = (v: any) => v == null ? "—" : new Intl.NumberFormat("es-CO", {
  style: "currency", currency: "COP", maximumFractionDigits: 0,
}).format(Number(v));

const dt = (v: any) => v ? new Intl.DateTimeFormat("es-CO", {
...
.top{background:var(--g);color:#fff;padding:18px 26px;display:flex;justify-content:space-between;gap:14px;align-items:center}.ey{font-size:10px;letter-spacing:1.4px;font-weight:800;opacity:.72}.top h1{margin:2px 0}
main{padding:20px 26px 45px;max-width:1900px;margin:auto}.nav,.actions,.filters,.hint-summary{display:flex;gap:7px;flex-wrap:wrap;align-items:center}
.btn,button,input,textarea,select{border:1px solid var(--l);border-radius:8px;padding:8px 10px;background:#fff;font:inherit}.btn,button{font-weight:750;cursor:pointer}.primary{background:var(--g2)!important;border-color:var(--g2)!important;color:#fff!important}.danger{background:#fff5f5!important;border-color:#d9aaaa!important;color:var(--r)!important}
.notice{padding:12px 14px;border:1px solid #ead99c;background:#fff3cf;color:#65490d;border-radius:10px;margin-bottom:14px}.ok{background:#e1f2e6;border-color:#b8dbc3;color:#175935}.bad{background:#f4e5e5;border-color:#dfbbbb;color:var(--r)}
.panel,.metric{background:#fff;border:1px solid var(--l);border-radius:13px}.metrics{display:grid;grid-template-columns:repeat(6,1fr);gap:10px;margin-bottom:14px}.metric{padding:14px}.metric span,.sub,label span{display:block;color:var(--m);font-size:11px}.metric strong{font-size:22px}
.head{padding:15px 17px;border-bottom:1px solid var(--l);display:flex;justify-content:space-between;gap:12px;align-items:center}.head h2{margin:0}.section{padding:16px}.tablewrap{overflow:auto}table{border-collapse:collapse;width:100%;min-width:1200px}th,td{padding:9px 10px;border-bottom:1px solid #edf0ec;text-align:left;white-space:nowrap;vertical-align:top}th{font-size:10px;text-transform:uppercase;color:var(--m);background:#fafbf9}.vehicle,.wrap{white-space:normal}.vehicle{min-width:330px}
.pill{font-size:10px;font-weight:800;border-radius:99px;padding:5px 8px;display:inline-block}.reviewed,.match,.hint-consistent{background:#daf0e1;color:#175935}.draft,.ambiguous,.notstated,.hint-unknown{background:#fff0c9;color:#72510a}.none{background:#ecefec;color:#586158}.conflict,.blocked,.hint-differs{background:#f4dfdf;color:#7f2424}.medium,.hint-compatible{background:#e8eef3;color:#425c70}
.login{width:min(430px,92vw);margin:15vh auto;background:#fff;border:1px solid var(--l);border-radius:16px;padding:28px}.login input{width:100%}.login button{width:100%;margin-top:10px}
.candidate-grid{display:grid;grid-template-columns:repeat(3,minmax(300px,1fr));gap:10px}.candidate-card{border:1px solid var(--l);border-radius:11px;padding:13px;background:#fafbf9}.candidate-card.selected{border:2px solid var(--g2);background:#f2faf5}.candidate-card h3{margin:0 0 5px}
...
@media(max-width:1200px){.candidate-grid{grid-template-columns:1fr 1fr}.two{grid-template-columns:1fr}.metrics{grid-template-columns:repeat(3,1fr)}}@media(max-width:760px){main{padding:13px}.top{align-items:flex-start}.candidate-grid,.evidence-grid,.metrics{grid-template-columns:1fr}.hint-row{grid-template-columns:1fr}}
`;
...
function login(error = false, lot = "") {
  return html("SUPERBID — Fasecolda Candidate Cockpit", `<form class="login" method="post" action="/functions/v1/superbid-fasecolda-candidate-cockpit/login">
    ...
    <div class="ey">SUPERBID · v0.53</div><h1>Candidate Resolution Cockpit</h1>
    <p class="sub">Acceso privado. La confirmación exacta exige evidencia humana de identidad; las pistas automáticas solo ayudan a leer texto y no generan homologación ni señal de compra.</p>
...
async function board(req: Request) {
  ...
  return html("SUPERBID — Candidate Resolution", `<header class="top"><div><div class="ey">SUPERBID · v0.53</div><h1>Fasecolda Candidate Resolution Cockpit</h1></div>${nav}</header><main><div class="notice"><strong>MANUAL_FASECOLDA_CANDIDATE_EVIDENCE_NOT_AUTOMATIC_MATCH_OR_BUY_SIGNAL.</strong> El score fuzzy organiza candidatos, pero no decide versión exacta. Confirmar exige seis dimensiones humanas, cero conflictos y al menos un MATCH no-lineal marcado explícitamente como discriminador frente a las alternativas actuales. Ningún candidato se preselecciona automáticamente.</div><div class="notice"><strong>${esc(IDENTITY_HINT_GUARDRAIL)}.</strong> Las pistas automáticas de texto son read-only: no se guardan como evidencia, no prellenan el formulario humano y no crean ranking, homologación ni recomendación.</div><section class="metrics">${cards}</section><section class="panel"><div class="head"><div><h2>Cola candidate-resolution</h2><span class="sub">Solo casos cuyo workflow canónico actual es CANDIDATE_RESOLUTION.</span></div>${filters}</div><div class="tablewrap"><table><thead><tr><th>Prioridad</th><th>Vehículo</th><th>Automático</th><th>Candidatos</th><th>Evidencia</th><th>Cierre</th><th>Acción</th></tr></thead><tbody>${tableRows || '<tr><td colspan="7">Sin casos para este filtro.</td></tr>'}</tbody></table></div></section></main>`);
}
...
async function detail(req: Request, lot: string) {
  ...
  const [auto, candidates, manual, evidence, attachments, history] = await Promise.all([
    ...
    one(`/rest/v1/lot_fasecolda_candidate_resolution_evidence?select=*&lot_id=eq.${encodeURIComponent(id)}&limit=1`),
    rows(`/rest/v1/lot_attachments?select=${encodeURIComponent("id,name,url,kind,source,discovered_at")}&lot_id=eq.${encodeURIComponent(id)}&order=kind.asc,id.asc`),
