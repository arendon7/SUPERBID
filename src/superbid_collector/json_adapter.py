...
def _walk(obj: Any) -> Iterable[dict]:
    """Yield every dict contained in arbitrary JSON."""
...
def looks_like_offer(d: dict) -> bool:
    ...
    has_auction_signal = any(k in d for k in ("offer_detail", "offer_status", "total_bids", "total_bidders", "lot_number", "end_date", "winner_bid"))
...
def offer_dict_to_observation(d: dict, url: str | None = None) -> LotObservation:
    ...
    initial = _num(offer_detail.get("initial_bid_value"))
    current = (_num(d.get("winner_bid")) or _num(d.get("price")) or _num(offer_detail.get("current_max_bid")) or _num(offer_detail.get("current_min_bid")))
    outcome = _outcome(d)
    evidence = {
        "parser": "superbid_json_v1",
        "source_fields": sorted(k for k in ("price", "winner_bid", "offer_detail", "offer_status", "total_bids", "total_bidders", "end_date", "auction") if k in d),
    ...
    return LotObservation(
        ...
        displayed_price_label="winner_bid" if d.get("winner_bid") is not None else "price/current_bid",
