...
async def capture_due(store: Store):
    ...
    for q in rows:
        ...
        try:
            r=await capture_public_json(q["url"],seconds=CAPTURE_SECONDS,db=store.path)
            matched=None
            ...
            # pydantic json mode serializes enums to string.
            mark_queue_result(
                store.conn,q["external_lot_id"],ok=True,
                closes_at_text=(matched or {}).get("closes_at_text"),
                outcome=outcome,
            )
            finish_run(
                store.conn,rid,ok=True,
            ...
            results.append({"id":q["external_lot_id"],"ok":True})
        except Exception as exc:
            mark_queue_result(
                store.conn,q["external_lot_id"],ok=False,error=str(exc)
            )
...
async def discovery_cycle(store: Store):
    ...
    for src in sources(store):
        try:
            r=await discover_from_source(store,src["url"],CAPTURE_SECONDS,src.get("source_type") or "listing")
...
def main():
    ...
    store.init()
    last_discovery=0.0
    print(json.dumps({"worker":"v2_started","db":DB},ensure_ascii=False))
    while True:
        now=time.time()
