...
ADMIN_TOKEN=os.getenv("SUPERBID_ADMIN_TOKEN","").strip();DASHBOARD_TOKEN=os.getenv("SUPERBID_DASHBOARD_TOKEN","").strip();DASHBOARD_COOKIE="superbid_dashboard_token"
def _token_from_request(request:Request)->str:
    auth=request.headers.get("authorization") or ""
    if auth.lower().startswith("bearer "):return auth[7:].strip()
    return (request.headers.get("x-superbid-token") or request.query_params.get("token") or request.cookies.get(DASHBOARD_COOKIE) or "").strip()
