...
DASH = (ROOT / "supabase/functions/superbid-fasecolda-search-dashboard/index.ts").read_text(encoding="utf-8").lower()
...
def test_board_reads_v034_diagnostics_without_writing():
    ...
    assert "fasecolda_unmatched_diagnostic_not_match" in DASH
...
def test_probe_precedes_override_confirmation():
    ...
    assert "const confirm=pr.has_codes?" in DASH
    ...
    assert "esto no fuerza un match high" in DASH
...
def test_ui_repeats_search_term_guardrails():
    ...
    assert "el término manual solo cambia la búsqueda" in DASH
